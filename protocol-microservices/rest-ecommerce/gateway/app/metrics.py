...
- metrics : cheap, pre-aggregated time-series scraped by Prometheus every few
            seconds (this module). They answer "how is the gateway doing right
...
("/api/users/{path}") and never by the raw path ("/api/users/9f3c-..."). 
Labelling by raw path — or by user id, request id, etc. — would mint a brand-new
