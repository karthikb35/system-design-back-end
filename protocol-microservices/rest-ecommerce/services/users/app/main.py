"""Application entry point — assembles the FastAPI app for the Users service.

Wiring order: configure logging -> create app -> add middleware -> include
routers -> create tables on startup (demo convenience).
"""
...
async def lifespan(app: FastAPI):
    ...
    if settings.auto_create_schema:
        await init_models()
    yield
    # Shutdown: nothing to clean up here (engine disposes on process exit).


app = FastAPI(
    title="Users Service",
    version="1.0.0",
    description="Owns user accounts and authentication for the e-commerce system.",
    lifespan=lifespan,
)
