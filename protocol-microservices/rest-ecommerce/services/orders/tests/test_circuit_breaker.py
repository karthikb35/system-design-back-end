...
def test_opens_after_threshold_consecutive_failures():
    ...
    # While OPEN (and before the cooldown elapses) calls are rejected fast.
...
def test_success_resets_failure_count():
    ...
    cb.record_failure()
    cb.record_failure()
    assert cb.state == "closed"  # only 2 in a row since the reset


def test_half_open_trial_closes_on_success():
    # recovery_time=0 => the cooldown has already elapsed, so the next allow()
    # promotes OPEN -> HALF_OPEN and admits a single trial call.
