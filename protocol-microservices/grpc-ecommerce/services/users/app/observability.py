...
The HTTP world uses middleware; gRPC uses *interceptors*. This one does the same
job as the REST correlation-id middleware:

  * read ``x-request-id`` from the call metadata (or mint a new one),
  * stash it in a contextvar so every log line carries it,
  * log one structured line per RPC with its status and latency.

All our RPCs are unary-unary, so we only need to wrap that handler kind.
"""
...
class CorrelationInterceptor(grpc.aio.ServerInterceptor):
    """Assigns/propagates a correlation id and logs one line per RPC."""
