...
the RPC FULL METHOD ("/ecommerce.users.UserService/GetUser") and the gRPC status
code NAME ("OK", "NOT_FOUND", ...), both of which are BOUNDED by the service
definition. Never label by request fields (user id, email, ...) — that would
mint a new series per value and exhaust Prometheus.
"""
