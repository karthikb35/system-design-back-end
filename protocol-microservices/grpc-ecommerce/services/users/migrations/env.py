"""Alembic migration environment — Users service (async engine, gRPC edition).

DB-per-service: this env targets ONLY the users database. `target_metadata` is
the users' `Base.metadata`, so `alembic revision --autogenerate` diffs against
the users models exclusively. The database URL comes from the service Settings
(app.config), so migrations always hit the same database the app uses;
...
# service directory (services/users/). Belt-and-suspenders alongside the
