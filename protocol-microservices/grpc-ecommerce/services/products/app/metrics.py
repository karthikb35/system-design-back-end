...
the RPC FULL METHOD ("/ecommerce.products.ProductService/GetProduct") and the
gRPC status code NAME ("OK", "NOT_FOUND", ...), both of which are BOUNDED by the
service definition. Never label by request fields (product id, sku, ...) — that
would mint a new series per value and exhaust Prometheus.
"""
