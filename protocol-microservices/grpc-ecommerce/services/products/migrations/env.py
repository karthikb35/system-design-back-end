"""Alembic migration environment — Products service (async engine, gRPC edition).

DB-per-service: this env targets ONLY the products database. `target_metadata`
is the products' `Base.metadata`, so `alembic revision --autogenerate` diffs
against the products models exclusively. The database URL comes from the service
Settings (app.config), so migrations always hit the same database the app uses;
docker-compose / production set DATABASE_URL to the real Postgres instance.

The service speaks gRPC, not HTTP, but that is invisible here: schema management
is a property of the database, not the transport, so this env.py is identical in
spirit to the REST edition.
...
# service directory (services/products/). Belt-and-suspenders alongside the
