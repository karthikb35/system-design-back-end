...
Placing an order touches two other services:
...
The clients are injected, so tests can point them at fake in-process servers.
"""
from __future__ import annotations

import logging

from .clients import ProductsGrpcClient, ProductUnavailable, UsersGrpcClient
...
class OrderService:
    def __init__(
        ...
        users: UsersGrpcClient,
        products: ProductsGrpcClient,
    ) -> None:
        self._repo = repo
        self._users = users
    ...
    async def place_order(self, user_id: str, items: list[tuple[str, int]]) -> Order:
        ...
        # 2) Price + reserve each line. Reserving stock is a remote mutation on
        # the Products service, so this loop is a mini distributed transaction. If
        # a later line is oversold, or persisting the order fails, the units we
        # already reserved would leak. We therefore track every reservation and,
        # on ANY failure, compensate by releasing them — the Saga pattern.
        ...
        try:
            ...
            # consume stock, then re-raise so the original error still reaches the
            # servicer (ProductUnavailable -> FAILED_PRECONDITION, etc.).
