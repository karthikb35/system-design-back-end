"""Alembic migration environment — Orders service (async engine, gRPC edition).

DB-per-service: this env targets ONLY the orders database. `target_metadata` is
the orders' `Base.metadata`, so `alembic revision --autogenerate` diffs against
the orders models exclusively. The database URL comes from the service Settings
...
The service speaks gRPC (and is itself a gRPC client of Users/Products), but
that is invisible here: schema management is a property of the database, not the
transport, so this env.py is identical in spirit to the REST edition.
