...
The Gateway is the only HTTP surface in this gRPC system, so it is the natural
home for an HTTP ``/metrics`` endpoint; the backend services expose their own
RED metrics over a gRPC interceptor instead (see each service's app/metrics.py).
...
("/users/{user_id}") and never by the raw path ("/users/9f3c-..."). Labelling
by raw path — or by user id, request id, email, etc. — would mint a brand-new
