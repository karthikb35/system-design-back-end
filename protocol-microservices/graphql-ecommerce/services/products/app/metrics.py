...
("/graphql", "/health") and never by the raw path. Labelling by raw path — or
by product id, request id, sku, etc. — would mint a brand-new series per value,
exhausting memory and slowing every query. Keep label sets small and BOUNDED
(method, template, status class).

GRAPHQL NOTE
------------
Almost every GraphQL request is a single ``POST /graphql``, so the RED metrics
below aggregate all queries and mutations under that one template — which is
exactly what we want for HTTP-level health (endpoint rate, error rate, latency).
Per-operation breakdowns (metrics keyed by the GraphQL operation/field name)
belong in a Strawberry schema extension, not in HTTP middleware, and are out of
scope here.
"""
