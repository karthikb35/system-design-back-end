...
by user id, request id, order id, etc. — would mint a brand-new series per
value, exhausting memory and slowing every query. Keep label sets small and
BOUNDED (method, template, status class).

GRAPHQL NOTE
------------
Almost every request through the gateway is a single ``POST /graphql``, so the
RED metrics below aggregate all queries and mutations under that one template —
